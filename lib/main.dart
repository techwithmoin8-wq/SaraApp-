...
class InvRow {
  ...
  static InvRow fromJson(Map<String, dynamic> j) =>
      InvRow(j['desc'], j['hsn'], (j['qty'] as num).toDouble(),
...
class AppState extends ChangeNotifier {
  ...
  void inwardRaw(String name, String uom, double qty, {double? unitCost}){
    ...
    if (i>=0){ raw[i].qty += qty; if(unitCost!=null) raw[i].unitCost = unitCost; }
    else { raw.add(StockItem(name: name, uom: uom, qty: qty, unitCost: unitCost??0)); }
    save(); notifyListeners();
  }
...
class LoginPage extends StatefulWidget{
  final AppState state; const LoginPage({super.key, required this.state});
...
class Dashboard extends StatefulWidget{
  final AppState state; const Dashboard({super.key, required this.state});
...
class HomeTab extends StatelessWidget{
  final AppState s; const HomeTab({super.key, required this.s});
  @override
  Widget build(BuildContext context){
    return AnimatedBuilder(
      animation: s,
...
class InvoiceTab extends StatefulWidget{
  final AppState s; const InvoiceTab({super.key, required this.s});
...
class _InvoiceTabState extends State<InvoiceTab>{
  ...
  final cgst = TextEditingController(text:"9"), sgst = TextEditingController(text:"9");
      ...
      build: (c)=> pw.Column(
        ...
        children: [
          ...
          pw.Table.fromTextArray(
            ...
            data: rows.map((r)=> [r.desc,r.hsn,r.qty.toStringAsFixed(2), r.rate.toStringAsFixed(2), (r.qty*r.rate).toStringAsFixed(2)]).toList(),
  ...
  Widget build(BuildContext context){
    ...
    return Stack(children:[
            ...
            Card(child: Padding(
              ...
              child: Row(children: [
                Expanded(flex: 30, child: TextField(
                  ...
                  onChanged: (v){ rows[i].desc=v; _persist(); setState((){}); },
                  style: txt, decoration: const InputDecoration(contentPadding: pad),
                )),
                const SizedBox(width: 6),
                Expanded(flex: 14, child: TextField(
                  controller: TextEditingController(text: rows[i].hsn),
                  onChanged: (v){ rows[i].hsn=v; _persist(); },
                  textAlign: TextAlign.center, style: txt,
                  decoration: const InputDecoration(contentPadding: pad),
                )),
                const SizedBox(width: 6),
                Expanded(flex: 18, child: TextField(
                ...
                IconButton(onPressed: (){ setState(()=> rows.removeAt(i)); _persist(); }, icon: const Icon(Icons.delete_outline)),
              ]),
            )),
      ...
      // action buttons (not overlapping totals)
      Positioned(
        bottom: 12, right: 12,
        child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          FilledButton.icon(onPressed: (){
            setState(()=> rows.add(InvRow("Water Bottle","373527",1,10))); _persist();
...
class OrdersTab extends StatelessWidget{
  final AppState s; const OrdersTab({super.key, required this.s});
  @override
  Widget build(BuildContext context){
    final cust = TextEditingController(), qty = TextEditingController(text:"100");
    return AnimatedBuilder(
      animation: s,
...
class StockTab extends StatelessWidget{
  final AppState s; const StockTab({super.key, required this.s});
  @override
  Widget build(BuildContext context){
    final name=TextEditingController(), uom=TextEditingController(text:"pcs");
    final qty=TextEditingController(text:"0"), cost=TextEditingController(text:"0");
    return AnimatedBuilder(
      animation: s,
      builder: (_, __)=> Scaffold(
        floatingActionButton: FloatingActionButton.extended(
...
class MaterialsTab extends StatelessWidget{
  final AppState s; const MaterialsTab({super.key, required this.s});
  @override
  Widget build(BuildContext context){
...
class AccountsTab extends StatefulWidget{
  final AppState s; const AccountsTab({super.key, required this.s});
...
class _AccountsTabState extends State<AccountsTab>{
  ...
  @override
  Widget build(BuildContext context){
    final s = widget.s; final cur = NumberFormat.currency(locale:"en_IN", symbol:"₹");
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        onPressed: (){
          ...
          showDialog(context: context, builder: (_)=> StatefulBuilder(builder: (_, setL)=> AlertDialog(
